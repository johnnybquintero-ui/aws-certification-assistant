...
def test_extract_operations_returns_a_list(
    fake_service_model,
    fake_service_name
):
...
def test_extract_operations_contains_correct_values(
    ...
    for record in result:
        ...
        assert record["service_name"] == fake_service_model["metadata"]["serviceFullName"]
        assert record["api_version"] == fake_service_model["metadata"]["apiVersion"]
        assert record["operation"] in fake_service_model["operations"]
        assert record["description"] == fake_service_model["operations"][record["operation"]].get("documentation", "")
...
def test_extract_operations_uses_empty_string_when_documentation_missing(
    ...
    fake_service_model["operations"]["CreateBucket"].pop(
        "documentation"
    )
